...
#ifndef __NGA_TIME_UTILS_HPP__
#define __NGA_TIME_UTILS_HPP__ 1
...
namespace nga {
    ...
    size_t durationToString(const std::chrono::duration<T, R> & duration, char * NGA_NONNULL buff, size_t buffSize) {
        if (buffSize < 20) {
            return 0;
        }
    ...
    template <typename S>
    size_t durationToString(const std::chrono::duration<S> & duration, char * NGA_NONNULL buff, size_t buffSize) {
        return durationToString<std::chrono::milliseconds>(std::chrono::duration_cast<std::chrono::milliseconds>(duration), buff, buffSize);
    }
    
} // namespace nga

#endif //!__NGA_TIME_UTILS_HPP__
