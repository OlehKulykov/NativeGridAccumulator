...
#ifndef __NGA_UTILS_HPP__
#define __NGA_UTILS_HPP__ 1
...
namespace nga {
    ...
    template<typename T>
    bool isPointerAligned(const void * NGA_NONNULL ptr, const T alignment = alignof(T)) noexcept {
        if (!ptr) {
            return true;
        }
        if (alignment == 0) {
