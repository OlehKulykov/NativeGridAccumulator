...
namespace crypto {
    ...
    struct BIODeleter final {
        void operator()(BIO * NGA_NULLABLE bio) const noexcept {
            if (bio) {
                ::BIO_free_all(bio);
            }
        }
    ...
    struct EVP_MACDeleter final {
        void operator()(EVP_MAC * NGA_NULLABLE mac) const noexcept {
            if (mac) {
                ::EVP_MAC_free(mac);
            }
        }
    ...
    struct EVP_MAC_CTXDeleter final {
        void operator()(EVP_MAC_CTX * NGA_NULLABLE ctx) const noexcept {
            if (ctx) {
    ...
    struct EVP_MD_CTXDeleter final {
        void operator()(EVP_MD_CTX * NGA_NULLABLE ctx) const noexcept {
            if (ctx) {
                ::EVP_MD_CTX_free(ctx);
            }
        }
    ...
    struct EVP_MDDeleter final {
        void operator()(EVP_MD * NGA_NULLABLE md) const noexcept {
            if (md) {
                ::EVP_MD_free(md);
            }
        }
    ...
    SHA256Buffer sha256(const void * NGA_NONNULL data, const size_t dataSize) {
    ...
    HMAC512Buffer hmacSha512(const void * NGA_NONNULL key, const size_t keySize,
    ...
    ZeroFillString base64Encode(const void * NGA_NONNULL data, const size_t dataSize) {
    ...
    ZeroFillDataVector base64Decode(const void * NGA_NONNULL data, const size_t dataSize) {
        if (dataSize == 0) return {};
