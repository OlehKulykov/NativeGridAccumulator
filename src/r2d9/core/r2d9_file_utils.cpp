...
#define __R2D9_FILE__ "FileUtils"

#include "r2d9_file_utils.hpp"
...
namespace file {
    ...
    RawHeapMemory readContent(const char * R2D9_NONNULL path, const size_t chunkSize) {
        ...
        size_t done = 0;
        uint8_t * buff = static_cast<uint8_t *>(content);
        while (done < size) {
    ...
    pid_t readPID(const char * R2D9_NONNULL path) {
    ...
    void writePID(const char * R2D9_NONNULL path) {
