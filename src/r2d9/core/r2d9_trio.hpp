...
namespace std {
    ...
    template<typename T1, typename T2, typename T3>
    struct hash<r2d9::TrioPOD<T1, T2, T3> > {
        size_t operator()(const r2d9::TrioPOD<T1, T2, T3> & trio) const {
    ...
    template<class T1, class T2, class T3>
    struct hash<r2d9::Trio<T1, T2, T3> > {
        size_t operator()(const r2d9::Trio<T1, T2, T3> & trio) const {
            return (std::hash<T1>()(trio.first)) ^
                   (((std::hash<T2>()(trio.second)) << 1) >> 1) ^
                   (std::hash<T3>()(trio.third) << 1);
        }
