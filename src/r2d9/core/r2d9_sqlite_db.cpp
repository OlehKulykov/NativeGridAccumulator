...
#define __R2D9_FILE__ "SQLite"

#include "r2d9_fixed_string_stream.hpp"

#include <sqlite3.h>

#include "r2d9_sqlite_db.hpp"
...
namespace r2d9 {
    ...
    void SQLiteDB::open(const char * R2D9_NONNULL path, const int flags) {
        DBUPtr db;
        ...
        if (res != SQLITE_OK) {
            char reason[256];
            ::snprintf(reason, 256, "SQLite open db, result: %i, error: %s", res, (::sqlite3_errmsg(db.get()) ?: ""));
    ...
    SQLiteDB::StmtUPtr SQLiteDB::prepare(const char * R2D9_NONNULL sql) {
        ...
        if (!stmtPtr || (res != SQLITE_OK)) {
            char reason[256];
            ::snprintf(reason, 256, "SQLite prepare statement, result: %i, error: %s", res, (::sqlite3_errmsg(_db.get()) ?: ""));
            throw std::runtime_error(reason);
        }
        return stmt;
    }
    ...
    int SQLiteDB::step(StmtUPtr & stmt) {
        ...
        if (res != SQLITE_OK) {
            ...
            char reason[256];
            ::snprintf(reason, 256, "SQLite step, result: %i, error: %s", res, (::sqlite3_errmsg(_db.get()) ?: ""));
            throw std::runtime_error(reason);
        }
    ...
    void SQLiteDB::reuse(StmtUPtr & stmt) {
        ...
        if (res != SQLITE_OK) {
            char reason[256];
            ::snprintf(reason, 256, "SQLite reuse statement, result: %i, error: %s", res, (::sqlite3_errmsg(_db.get()) ?: ""));
            throw std::runtime_error(reason);
        }
    }
    
    void SQLiteDB::execute(const char * R2D9_NONNULL sql) {
        char * errMsg = nullptr;
        const int res = ::sqlite3_exec(_db.get(), sql, nullptr, nullptr, &errMsg);
        if (res != SQLITE_OK) {
            char reason[256];
            ::snprintf(reason, 256, "SQLite execute, result: %i, error: %s", res, (errMsg ?: ""));
